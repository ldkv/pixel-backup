...
function renderUserConfigRow(uc) {
  ...
  `;
  tr.querySelector('[data-action="edit"]').addEventListener('click', () => renderUserConfigEditRow(tr, uc));
  tr.querySelector('[data-action="remove"]').addEventListener('click', () => deleteUserConfig(uc.id));
  return tr;
}
...
function renderUserConfigEditRow(tr, uc) {
  ...
  tr.querySelector('[data-action="save"]').addEventListener('click', async () => {
    const body = {
      ...
      sync_order: parseInt(tr.querySelector('[data-field="sync_order"]').value, 10),
    };
...
async function loadBatches() {
  ...
  const url = username ? `/batches?username=${encodeURIComponent(username)}` : '/batches';
  const res = await fetch(url);
  const batches = await res.json();
  const body = document.getElementById('batches-body');
  body.innerHTML = '';
  if (batches.length === 0) {
    body.innerHTML = '<tr class="empty-row"><td colspan="4">No batches yet.</td></tr>';
    return;
  }
...
document.getElementById('add-user-config-form').addEventListener('submit', async (e) => {
  ...
  const body = {
    ...
    sync_order: parseInt(document.getElementById('uc-sync-order').value, 10),
  };
  const res = await fetch('/user_configs', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
